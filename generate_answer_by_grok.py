...
# Changed from "cookies" to "grok_cookies"
COOKIES_DIR = Path("grok_cookies")
encrypted_files = list(COOKIES_DIR.glob("*.encrypted"))

if not encrypted_files:
    raise RuntimeError("❌ No .encrypted cookie files found in 'grok_cookies/' folder")
...
def run():
    ...
    try:
        ...
        # Changed URL to grok.com
        print("[STEP] Opening Grok Main URL...", flush=True)
        page.goto(
            "https://grok.com/",
            wait_until="load"
        )
        ...
        # New Target Textbox Locator
        textbox = page.get_by_role('textbox', name='Ask Grok anything')
        
        if textbox.count() > 0:
            textbox.first.click()
            print("[OK] Textbox located and clicked successfully.", flush=True)
        else:
            raise RuntimeError("❌ Textbox locator load nahi ho paya (Grok strategy failed).")
            
        custom_random_wait(15, 30)

        # ============================================
        # PROMPT FOR SUBSTACK NOTES
        # ============================================
        ...
        # New Target Submit Button Locator
        send_button = page.get_by_test_id('chat-submit')
        send_button.click()
        ...
        # New Code Block Locator targeting 'pre'
